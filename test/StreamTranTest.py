...
#ql = randn(10)
#ql = ql+abs(min(ql))
q_lin = CreateLatFlowVec(ql,nx)
x = np.linspace(0,L,num=nx)

#try and initialize for a flow only
sTran = StreamTranSim('test')
sTran.L = L
sTran.nx = nx
sTran.wi = w
#sTran.d = d
...
Ai = InterpolateMatrialValue(A[0],A[1],nx,L)
figure()
plot(x,Ai)
xlabel('distance')
ylabel('area')
...
Qtr = InterpolateTribPump(Q_trib[0],Q_trib[1],nx,L)
figure()
plot(x,Qtr)
xlabel('distance')
...
Ctr = InterpolateTribPump(Q_trib[0],Q_trib[2],nx,L)
figure()
plot(x,Ctr)
xlabel('distance')
...
numSteps = len(ql)
Cgw = InterpolateGwConc(C_gw[0],C_gw[1],numSteps,nx,L)
figure()
plot(x,Cgw)
xlabel('distance')
ylabel('GW conc.')
...
Cgw = InterpolateGwConcTau(C_tau[0],C_tau[1],numSteps,nx,L,df_cfc,'CFC11',df_cfc.index[-1],0.,mod_type='exponential')
figure()
plot(x,Cgw)
xlabel('distance')
ylabel('GW conc.')
